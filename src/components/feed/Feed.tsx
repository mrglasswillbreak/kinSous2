...
function useLiveBounties(
  ...
  const fetchBounties = useCallback(
    async (
      ...
      try {
        ...
        if (q) params.set("q", q);
        if (nextScope === "MY_BOUNTIES" && nextUserId)
          params.set("seekerId", nextUserId);
        if (nextScope === "MY_BIDS" && nextUserId)
          params.set("helperId", nextUserId);
        const res = await fetch(`/api/bounties?${params}`, {
          signal: controller.signal,
        });
        if (!res.ok)
          throw new Error("Could not load bounties. Please try again.");
        if (res.ok) {
  ...
  return {
    ...
    loading,
    error,
    refetch: () => fetchBounties(category, query, scope, userId),
  };
}

export default function Feed() {
  const { user } = useCurrentUser();
  const [activeCategory, setActiveCategory] = useState<BountyCategory | "ALL">(
    "ALL",
  );
  ...
  const { bounties, loading, error, refetch } = useLiveBounties(
    activeCategory,
    searchQuery,
    activeScope,
    user?.userId,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted text-sm mt-0.5">
              {loading
                ? "Loading…"
                : `${displayed.length} request${displayed.length !== 1 ? "s" : ""} available`}
            </p>
          </div>
          <motion.button
            whileTap={{ scale: 0.92 }}
